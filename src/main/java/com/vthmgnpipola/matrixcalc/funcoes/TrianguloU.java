...
@FuncaoRegistrada("triu")
public class TrianguloU implements ManagerFunctions.Input1 {
    @Override
    public Operation.Info create(Variable A, ManagerTempVariables manager) {
        ...
        if (A instanceof VariableMatrix) {
            ...
            resultado.op = new Operation("triu-m") {
                @Override
                public void process() {
                    ...
                    output.matrix = lu.getUpper(null);
                }
            };
        } else {
            final VariableDouble output = manager.createDouble();
            resultado.output = output;
            resultado.op = new Operation("triu-s") {
                @Override
                public void process() {
